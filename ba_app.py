# Simulação de consultoria na Área de Dados com Agentes de IA
# Projeto Prático Para Consultoria na Área de Dados com Agentes de IA
# Deploy de App Para Day Trade Analytics em Tempo Real com Agentes de IA, Groq, OpenIA e AWS Para Monetização
...
def busca_dados(ticker, period="6mo"):
    ...
    hist = stock.history(period=period, timeout=60)
...
def ba_plot_stock_price(hist, ticker):
...
def ba_plot_candlestick(hist, ticker):
...
def ba_plot_media_movel(hist, ticker):
...
def ba_plot_volume(hist, ticker):
...
# Agentes de IA 
ba_agente_web_search = Agent(name="BA Agente Web Search",
...
ba_agente_financeiro = Agent(name="BA Agente Financeiro",
                              model=Groq(id="openai/gpt-oss-20b"),
...
ba_multi_ai_agent = Agent(team=[ba_agente_web_search, ba_agente_financeiro],
                       model=Groq(id="openai/gpt-oss-20b"),
...
st.set_page_config(page_title="Consultoria BA", page_icon=":bar_chart:", layout="wide")
...
### Finalidade da App:
Este aplicativo realiza análises avançadas de preços de ações da Nasdaq em tempo real utilizando Agentes de IA com modelo OpenIA através do Groq e infraestrutura AWS para apoio a estratégias de Day Trade para monetização. Uma app completa na Área de Dados e IA.
""")

# Botão de suporte na barra lateral
if st.sidebar.button("Suporte"):
    st.sidebar.write("No caso de dúvidas envie e-mail para: biasandrade@gmail.com")

# Título principal
st.title(":bar_chart: Consultoria BA")
...
if st.button("Analisar"):
    ...
    if ticker:
        ...
        with st.spinner("Buscando os Dados em Tempo Real. Aguarde..."):
            ...
            # Obtém os dados
            hist = busca_dados(ticker)
            
            # Renderiza um subtítulo
            st.subheader("Análise Gerada Por IA")
            
            # Executa o time de Agentes de IA
            ai_response = ba_multi_ai_agent.run(f"Resumir a recomendação do analista e compartilhar as últimas notícias para {ticker}")
            ...
            ba_plot_stock_price(hist, ticker)
            ba_plot_candlestick(hist, ticker)
            ba_plot_media_movel(hist, ticker)
            ba_plot_volume(hist, ticker)
    else:
